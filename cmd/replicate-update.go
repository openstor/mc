...
func checkReplicateUpdateSyntax(ctx context.Context, cmd *cli.Command) {
	if cmd.Args().Len() != 1 {
...
func modifyRemoteTarget(cmd *cli.Command, targets []madmin.BucketTarget, arnStr string) (*madmin.BucketTarget, []madmin.TargetUpdateType) {
	args := cmd.Args()
	foundIdx := -1
	...
	if foundIdx < 0 {
		fatalIf(errInvalidArgument().Trace(args.Slice()...), "`"+arnStr+"` not found in replication config")
	}
	...
	if cmd.IsSet("proxy") {
		...
		default:
			fatalIf(errInvalidArgument().Trace(args.Slice()...), "--proxy can be either [enable|disable]")
		}
	}
	...
	if len(args.Slice()) == 1 {
		...
		if tgtBucket != bktTarget.TargetBucket {
			fatalIf(errInvalidArgument().Trace(args.Slice()...), "configured remote target bucket `"+tgtBucket+"` does not match "+bktTarget.TargetBucket+"` for this ARN `"+bktTarget.Arn+"`")
		}
		if sourceBucket != bktTarget.SourceBucket {
			fatalIf(errInvalidArgument().Trace(args.Slice()...), "configured source bucket `"+sourceBucket+"` does not match "+bktTarget.SourceBucket+"` for this ARN `"+bktTarget.Arn+"`")
		}
...
func mainReplicateUpdate(ctx context.Context, cmd *cli.Command) error {
	...
	checkReplicateUpdateSyntax(ctx, cmd)
	...
	if cmd.IsSet("remote-bucket") {
		bktTarget, ops := modifyRemoteTarget(cmd, targets, arn)
